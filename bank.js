...
const account1 = {
  ...
  interestRate: 0.03,
  movTime: ["13,3,2025 10:14]", "13,3,2025 10:14"],
};
const account2 = {
  owner: "Siyum Teshome",
  pin: 2016,
...
const start = function () {
  ...
  //introduction
  name.textContent = "Hello, " + currentAccount.owner.split(" ")[0] + "!";
  // setting time
  logOut();
  const date = new Date();
...
const logOut = () => {
  ...
  setInterval(function () {
    timer.textContent = `${Math.trunc(h / 60)}:${h % 60}`;
    h = h - 1;
    if (h === 0) {
      main.style.opacity = 0;
      setTimeOut();
      timer.textContent = "log in again!";
    }
  }, 1000);
};
